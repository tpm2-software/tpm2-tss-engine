...
openssl pkeyutl -keyform engine -engine tpm2tss -inkey sm2key -sign -in mydata -out mysig

R="$(openssl pkeyutl -keyform engine -engine tpm2tss -inkey sm2key -verify -in mydata -sigfile mysig || true)"
